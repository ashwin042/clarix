...
<div x-data="{ sidebarOpen: true, mobileOpen: false, openMenu: null }" @keydown.escape.window="mobileOpen = false" class="flex h-screen overflow-hidden">
    ...
    {{-- Sidebar --}}
    <aside
        :class="[sidebarOpen ? 'md:w-60' : 'md:w-16', mobileOpen ? '!translate-x-0' : '']"
        class="fixed inset-y-0 left-0 z-40 w-60 -translate-x-full transform md:relative md:z-auto md:translate-x-0 flex flex-col flex-shrink-0 bg-white border-r border-gray-200 transition-all duration-300 ease-in-out overflow-hidden"
    >
        {{-- Logo --}}
        ...
        <nav class="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto overflow-x-hidden">
            ...
            {{-- Settings. Was a dead '#' placeholder; pointed at the account
                 forms now that they have a URL of their own. --}}
        ...
        <header class="flex items-center justify-between h-16 px-4 md:px-6 bg-white border-b border-gray-200 flex-shrink-0">
            <div class="flex items-center gap-3 md:gap-4">
                ...
                <button @click="mobileOpen = true; sidebarOpen = true"
                    class="md:hidden p-1.5 rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                ...
                <button @click="sidebarOpen = !sidebarOpen"
                    class="hidden md:block p-1.5 rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                    </svg>
                </button>
