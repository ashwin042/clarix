<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5 flex items-start justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 dark:text-slate-100 mt-1">{{ $stats['totalTasks'] }}</p>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">All time</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5 flex items-start justify-between">
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Completion Rate</p>
                <p class="text-3xl font-bold text-green-600 mt-1">{{ $stats['completionRate'] }}%</p>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $stats['completedTasks'] }} of {{ $stats['totalTasks'] }} tasks</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5 flex items-start justify-between">
            <div>
                <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Active Units</p>
                <p class="text-3xl font-bold text-gray-900 dark:text-slate-100 mt-1">{{ $stats['totalUnits'] }}</p>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">{{ $stats['totalUsers'] }} total users</p>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200 mb-4">Task Status</h3>
            <div class="relative h-52">
                <canvas id="statusDonut"></canvas>
            </div>
        </div>

        <div class="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200 mb-4">Tasks Created (Last 30 Days)</h3>
            <div class="h-52">
    ...
    {{-- Bar chart --}}
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none p-5">
        <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200 mb-4">Tasks per Unit</h3>
        <div class="h-52">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-800/60 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200">Recent Tasks</h3>
        ...
        <div class="divide-y divide-gray-50">
            ...
            @endphp
            <div class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <div class="flex items-center gap-3 ml-3 flex-shrink-0">
                    ...
                    <span class="text-xs text-gray-400 dark:text-slate-500 whitespace-nowrap">{{ $task->created_at->diffForHumans() }}</span>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm dark:shadow-none overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-800/60">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200">Activity Feed</h3>
        </div>
        <div class="p-5 space-y-4">
            @foreach($recentAssignments as $assignment)
            <div class="flex items-start gap-3">
                <div class="w-7 h-7 rounded-full bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <div class="flex items-start gap-3">
                <div class="w-7 h-7 rounded-full bg-teal-50 dark:bg-teal-500/10 flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            @if($recentAssignments->isEmpty() && $recentFiles->isEmpty())
                <p class="text-sm text-gray-400 dark:text-slate-500 text-center py-6">No recent activity.</p>
            @endif
        </div>
    </div>
