<div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 dark:text-slate-100">Rs {{ number_format($totalCredits, 2) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg {{ $netProfit >= 0 ? 'bg-emerald-100 dark:bg-emerald-500/10' : 'bg-red-100 dark:bg-red-500/10' }} flex items-center justify-center">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold {{ $netProfit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">Rs {{ number_format(abs($netProfit), 2) }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-amber-100 dark:bg-amber-500/10 flex items-center justify-center">
                ...
                <div>
                    <p class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Pending Credit</p>
                    <p class="text-xl font-bold text-amber-600 dark:text-amber-400">Rs {{ number_format(max(0, $pendingCredit), 2) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-800/60 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-slate-100">Recent Payments</h3>
        ...
        @if($recentPayments->count())
            <table class="min-w-full divide-y divide-gray-100 dark:divide-slate-800/60">
                <thead class="bg-gray-50 dark:bg-slate-950/60">
                    <tr>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase">Payer</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800/60">
                    @foreach($recentPayments as $p)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors">
                            ...
                            <td class="px-5 py-3 text-sm font-semibold text-green-600 dark:text-green-400">Rs {{ number_format($p->amount, 2) }}</td>
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-slate-300">{{ $p->unit?->name ?? '—' }}</td>
                            <td class="px-5 py-3 text-sm text-gray-600 dark:text-slate-300">{{ str_replace('_', ' ', ucfirst($p->payment_method)) }}</td>
                            <td class="px-5 py-3 text-sm text-gray-500 dark:text-slate-400">{{ $p->created_at->format('M d, Y') }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        @else
            <div class="py-12 text-center">
                <p class="text-sm text-gray-500 dark:text-slate-400">No payments recorded yet.</p>
            </div>
        @endif
    </div>
</div>
...
<script>
    ...
    const gridColor = isDark ? 'rgba(148,163,184,0.07)' : 'rgba(0,0,0,0.06)';
    ...
    new Chart(document.getElementById('revenueChart'), {
        ...
        options: {
            ...
            scales: {
                ...
                y: { ticks: { color: tickColor, callback: v => 'Rs ' + v.toLocaleString() }, grid: { color: gridColor } }
            }
        }
    });
