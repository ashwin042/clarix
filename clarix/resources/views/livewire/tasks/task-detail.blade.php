<div>
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            <div class="flex items-center gap-2 mb-1">
                <a href="{{ route('tasks.index') }}" class="text-sm text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-400 transition-colors">Tasks</a>
                <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-sm text-gray-600 dark:text-slate-400">{{ $task->task_code }}</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ $task->title }}</h1>
            <div class="flex items-center flex-wrap gap-x-4 gap-y-1 mt-2">
                ...
                <span class="text-sm {{ $task->deadline->isPast() && $task->status !== 'completed' ? 'text-red-600 font-medium' : 'text-gray-500 dark:text-slate-400' }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-5">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <h2 class="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-4">Details</h2>
                ...
                @if($task->important_notes)
                    <div class="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800/60">
                        <p class="text-xs text-gray-400 dark:text-slate-500 mb-1 font-medium uppercase tracking-wider">Important Notes</p>
                        <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{{ $task->important_notes }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-sm font-semibold text-gray-900 dark:text-slate-100">
                        Assigned Writers
                        <span class="ml-1.5 inline-flex items-center justify-center w-5 h-5 text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 rounded-full">{{ $task->assignments->count() }}</span>
                    </h2>
                    @can('assign', $task)
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        @if(auth()->user()->isWriter())
                                            <p class="text-sm font-medium text-gray-800 dark:text-slate-200">You</p>
                                        @elseif(auth()->user()->isPm())
                                            <p class="text-sm font-medium text-gray-800 dark:text-slate-200 text-gray-400 dark:text-slate-500 italic">Writer</p>
                                        @else
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @can('assign', $task)
                                        <button wire:click="removeAssignment({{ $assignment->id }})"
                                            ...
                                            class="p-1.5 text-gray-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                ...
                @else
                    <p class="text-sm text-gray-400 dark:text-slate-500">No writers assigned yet.</p>
                @endif
            </div>
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-950/50 rounded-xl">
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                <a href="{{ route('tasks.files.download', [$task, $file]) }}"
                                    class="px-3 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">Download</a>
                                @can('uploadCompletedFile', $task)
                                <button wire:click="deleteCompletedFile({{ $file->id }})"
                                    wire:confirm="Delete this completed file?"
                                    class="p-1.5 text-gray-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                </button>
                                @endcan
        ...
        <div class="space-y-5">
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <h2 class="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-4">Notes
                    <span class="ml-1.5 inline-flex items-center justify-center w-5 h-5 text-xs bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 rounded-full">{{ $task->notes->count() }}</span>
                </h2>
                    ...
                    <div class="space-y-3 mb-4 max-h-72 overflow-y-auto pr-1">
                        @foreach($task->notes as $n)
                            <div class="p-3 bg-amber-50 border border-amber-100 rounded-xl">
                                <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed">{{ $n->note }}</p>
                                <p class="text-xs text-gray-400 dark:text-slate-500 mt-2">{{ $n->author->name }} · {{ $n->created_at->diffForHumans() }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="text-sm text-gray-400 dark:text-slate-500 mb-4">No notes yet.</p>
                @endif
    ...
    >
        ...
        <div x-show="show" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            class="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800/60">
                <h3 class="text-base font-semibold text-gray-900 dark:text-slate-100">Assign Writers</h3>
                <button @click="show = false" class="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-400 p-1 hover:bg-gray-100 dark:hover:bg-slate-800/50 rounded-lg transition-colors">
            ...
            <div class="p-6">
                ...
                @if($availableWriters->count())
                    <div class="space-y-2 max-h-72 overflow-y-auto mb-4">
                        @foreach($availableWriters as $writer)
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 dark:border-slate-800/60 hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer transition-colors"
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-800 dark:text-slate-200">{{ $writer->name }}</p>
                                </div>
                ...
                @else
                    <p class="text-sm text-gray-500 dark:text-slate-400 text-center py-6">All available writers are already assigned.</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div
        ...
        x-init="if ({{ $errors->hasAny(['files', 'files.*']) ? 'true' : 'false' }}) show = true"
        x-show="show"
    ...
    >
        ...
        <div x-show="show" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            ...
            <form action="{{ route('tasks.files.store', $task) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4"
                @submit="if (fileObjects.length) uploading = true">
                @csrf
                {{-- Drag & Drop Zone --}}
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Note <span class="text-gray-400 dark:text-slate-500 font-normal">(optional)</span></label>
                    <textarea name="upload_note" rows="2" placeholder="Add a note about these files..."
                ...
                <div class="flex gap-3">
                    <button type="submit" :disabled="uploading"
                        class="flex-1 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
    ...
    >
        ...
        <div x-show="show" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            ...
            <form action="{{ route('tasks.completed-files.store', $task) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4"
                ...
                {{-- Upload progress (indeterminate) --}}
                <div x-show="uploading" x-cloak class="space-y-1.5">
                    <div class="upload-progress-track is-green"></div>
                ...
                <div class="flex gap-3">
                    <button type="submit"
                        :disabled="fileObjects.length === 0 || uploading"
                        class="flex-1 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
                        <svg x-show="uploading" x-cloak class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
                        </svg>
