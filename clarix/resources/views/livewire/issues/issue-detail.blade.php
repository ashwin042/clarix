<div>
    ...
    <div class="flex items-center gap-2 text-sm text-gray-400 dark:text-slate-500 mb-6">
        <a href="{{ route('issues.index') }}" class="hover:text-gray-600 dark:text-slate-400 transition-colors">Issues</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-gray-700 dark:text-slate-300 font-medium truncate max-w-xs">{{ $issue->title }}</span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-6">
                <div class="flex items-start justify-between gap-4 mb-4">
                    <h1 class="text-xl font-bold text-gray-900 dark:text-slate-100 leading-tight">{{ $issue->title }}</h1>
                    @php
                ...
                <div class="flex items-center gap-3 mb-5 text-xs text-gray-400 dark:text-slate-500 dark:text-slate-400">
                    <span class="inline-flex px-2 py-0.5 rounded-full font-medium {{ $pc }}">{{ ucfirst($issue->priority) }} Priority</span>
                    <span>Submitted by <span class="font-medium text-gray-600 dark:text-slate-400">{{ $issue->creator->name }}</span></span>
                    <span>{{ $issue->created_at->format('M d, Y') }}</span>
                </div>
                <div class="prose prose-sm max-w-none">
                    <p class="text-gray-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{{ $issue->message }}</p>
                </div>
            </div>

            {{-- Thread --}}
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-800/60">
                    <h2 class="text-sm font-semibold text-gray-800 dark:text-slate-200">Thread ({{ $issue->replies->count() }})</h2>
                </div>
                        ...
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 rounded-full {{ $isAdmin ? 'bg-indigo-600' : 'bg-gray-200' }} flex items-center justify-center">
                                    <span class="text-xs font-semibold {{ $isAdmin ? 'text-white' : 'text-gray-600 dark:text-slate-400' }}">
                                        {{ strtoupper(substr($reply->author->name, 0, 1)) }}
                                    </span>
                                </div>
                                <span class="text-sm font-medium text-gray-800 dark:text-slate-200">{{ $reply->author->name }}</span>
                                @if($isAdmin)
                                <span class="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-indigo-100 text-indigo-700">Admin</span>
                                @endif
                            </div>
                            <span class="text-xs text-gray-400 dark:text-slate-500 dark:text-slate-400">{{ $reply->created_at->format('M d, Y · H:i') }}</span>
                        </div>
                        <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap ml-9">{{ $reply->message }}</p>
                    </div>
                    @endforeach
                </div>
                @else
                <div class="px-6 py-8 text-center text-sm text-gray-400 dark:text-slate-500">No replies yet.</div>
                @endif

                {{-- Reply form --}}
                @if($issue->status !== 'closed')
                <div class="px-6 py-4 border-t border-gray-100 dark:border-slate-800/60 bg-gray-50 dark:bg-slate-950/50">
                ...
                @else
                <div class="px-6 py-4 border-t border-gray-100 dark:border-slate-800/60 text-center text-sm text-gray-400 dark:text-slate-500">This issue is closed.</div>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <h3 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Issue Details</h3>
                <dl class="space-y-3 text-sm">
                    ...
                    <div class="flex items-center justify-between">
                        <dt class="text-gray-500 dark:text-slate-400">Priority</dt>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <dd class="font-medium text-gray-700 dark:text-slate-300">{{ $issue->creator->name }}</dd>
                    </div>
                    <div class="flex items-center justify-between">
                        <dt class="text-gray-500 dark:text-slate-400">Role</dt>
                        <dd class="font-medium text-gray-700 dark:text-slate-300">{{ ucfirst($issue->creator->role) }}</dd>
                    </div>
                    <div class="flex items-center justify-between">
                        <dt class="text-gray-500 dark:text-slate-400">Opened</dt>
                        <dd class="text-gray-600 dark:text-slate-400">{{ $issue->created_at->format('M d, Y') }}</dd>
                    </div>
                    <div class="flex items-center justify-between">
                        <dt class="text-gray-500 dark:text-slate-400">Replies</dt>
                        <dd class="font-medium text-gray-700 dark:text-slate-300">{{ $issue->replies->count() }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-5">
                <h3 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Update Status</h3>
                <div class="space-y-2">
                    @php
                        $transitions = [
                    ...
                    @else
                    <p class="text-xs text-gray-400 dark:text-slate-500 text-center py-2">Issue is closed</p>
                    @endif

                    @if($issue->status !== 'open')
                    <button wire:click="updateStatus('open')"
                        class="w-full py-2 border border-gray-300 text-gray-600 dark:text-slate-400 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors">
                        Reopen Issue
                    </button>
                    @endif
                </div>
            ...
            {{-- Back link --}}
            <a href="{{ route('issues.index') }}" class="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:text-slate-300 transition-colors">
