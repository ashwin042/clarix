<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">Issues</h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5">{{ auth()->user()->isAdmin() ? 'All submitted issues' : 'Your submitted issues' }}</p>
        </div>
        ...
        @if($issues->count())
            <table class="min-w-full divide-y divide-gray-100 dark:divide-slate-800/60">
                <thead class="bg-gray-50 dark:bg-slate-950/60">
                    <tr>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Title</th>
                        @if(auth()->user()->isAdmin())
                        <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Submitted By</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800/60">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors">
                        <td class="px-5 py-3">
                            <p class="text-sm font-medium text-gray-900 dark:text-slate-100">{{ $issue->title }}</p>
                        </td>
                        @if(auth()->user()->isAdmin())
                        <td class="px-5 py-3 text-sm text-gray-600 dark:text-slate-400">{{ $issue->creator->name }}</td>
                        <td class="px-5 py-3">
                        ...
                        <td class="px-5 py-3 text-sm text-gray-500 dark:text-slate-400">{{ $issue->created_at->format('M d, Y') }}</td>
            ...
            @if($issues->hasPages())
                <div class="px-5 py-4 border-t border-gray-100 dark:border-slate-800/60">{{ $issues->links() }}</div>
            @endif
        @else
            <div class="py-16 text-center dark:text-slate-400">
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400">No issues found.</p>
                @if(!auth()->user()->isAdmin())
